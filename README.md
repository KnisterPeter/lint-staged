...
When supplying configuration in JS format it is possible to to define the linter command as a function which will receive an array of staged filenames/paths and should return the complete linter command as a string. It is also possible to return an array of complete command strings, for example when the linter command supports only a single file input.
